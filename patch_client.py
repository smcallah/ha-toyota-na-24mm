...
# From widewing/ha-toyota-na PR #182. 24MM vehicles can expose their current
# state, including EV/PHEV charging information, through AppSync.
...
async def graphql_get_vehicle_status(
    ...
    """
    Direct 24MM current-state query from PR #182.

    This is deliberately called as a standalone helper by
    patch_seventeen_cy_plus.py, so we do not need to modify __init__.py or
    disturb the working auth monkey-patches already installed.
    """
    headers = {
        "Content-Type": "application/json",
        "x-api-key": APPSYNC_API_KEY,
    ...
    payload = json.dumps(
        {
            ...
            "query": GRAPHQL_GET_VEHICLE_STATUS,
            "variables": {"vin": vin},
        }
    )

    _LOGGER.warning(
        "Toyota 24MM: requesting AppSync GetVehicleStatus for VIN ...%s",
        vin[-4:],
    )
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            ...
            if resp.status >= 400:
                _LOGGER.warning(
                    "Toyota 24MM GetVehicleStatus HTTP %d: %s",
                    resp.status,
                    body[:500],
                )
                return None
            ...
            try:
                ...
                _LOGGER.warning(
                    "Toyota 24MM GetVehicleStatus returned non-JSON: %s",
                    body[:500],
                )
                return None
            ...
            if result.get("errors"):
                err = result["errors"][0]
                _LOGGER.warning(
                    "Toyota 24MM GetVehicleStatus GraphQL error: %s: %s",
                    err.get("errorType"),
                    err.get("message"),
                )
                return None
            ...
            status = data.get("getVehicleStatus")

            if not status:
                _LOGGER.warning(
                    "Toyota 24MM GetVehicleStatus returned no vehicle status"
                )
                return None

            _LOGGER.warning(
                "Toyota 24MM GetVehicleStatus succeeded; top-level keys=%s",
                list(status.keys()),
            )
            ...
            _LOGGER.warning(
                "Toyota 24MM electric payload present=%s battery_keys=%s charging_keys=%s",
                bool(electric),
                list(battery.keys()),
                list(charging.keys()),
            )
